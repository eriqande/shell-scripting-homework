...
```sh
% [ -f $FILE ] && echo "Yes, the file, $FILE, exists" || echo "No, $FILE does not exist"
```
...
```sh
% if [ -f $FILE ]; then echo "Yes, the file, $FILE, exists"; else echo "No, $FILE DOES NOT EXIST"; fi
Yes, the file, ./inputs/sample_list.txt, exists

% FILE2=cats
% if [ -f $FILE2 ]; then echo "Yes, the file, $FILE2, exists"; else echo "No, $FILE2 DOES NOT EXIST"; fi
No, cats DOES NOT EXIST
```
...
```sh
for fq in ./all_fastqs/*.fq; do ./scripts/random-fail.sh ${fq} > ${fq/fq/counts} 2>> ./all_fastqs/error-messages.txt; done
```

## When you are done with that, print the lines in error-messages.txt
```sh
% wc ./all_fastqs/error-messagetxt
252    1119   14507 ./all_fastqs/error-messages.txt
```
